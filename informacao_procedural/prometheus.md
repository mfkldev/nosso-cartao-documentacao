...
É escrito principalmente em Go e licenciado sob a licença Apache 2.0, na qual existem centenas de pessoas que 
contribuíram para o projeto em si, que não são controladas por nenhuma empresa.

Demais né? Um projeto focado em métrica, código aberto e sem nenhuma empresa por trás, esses fatores o deixam muito 
...
do endereço `www.prd-servico-proposta.com.br/actuator/prometheus` a cada 5s ele fará!

Esse modelo é muito bom, pois a sua aplicação não tem a responsabilidade de enviar as métricas e sim de apenas fornecer 
no formato Prometheus e certamente ele (Prometheus) irá coletar!

Demais né! Conhecia esse modelo?

#### Push Model

O modelo push model é o menos utilizado, pois ele atende um cenário muito específico, como por exemplo, operações de 
